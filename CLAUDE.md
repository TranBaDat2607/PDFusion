...
4. **Every LLM backend's SDK call goes through `BaseTranslator._call_with_backoff(service, request)`**, never a bare `self.client...create(...)`. It acquires a token from `rate_limiter.get_rate_limiter(service)` — a process-wide singleton per service, shared across every job, sub-job and worker thread, with a per-service default (`_DEFAULT_QPS_BY_SERVICE`) overridable via `<service>.max_qps` in settings — then retries 429/5xx with jittered exponential backoff (`_MAX_RETRIES`, fatal 401/403 excluded). This only works because the SDK clients are constructed with `max_retries=0`: the OpenAI/Anthropic SDKs default to retrying twice on their own, and those inner attempts don't take a token, so leaving the default in place lets a 429 storm blow well past the configured QPS. A new backend must do the same, plus two more things every existing one does: check `self.is_cancelled()` at the very top of `translate()` (before the cache lookup — a cancelled job shouldn't even pay for a cache read) and add `except TranslationCancelled: return text` **before** `except Exception` — reversed, a cancelled paragraph gets funnelled into `_handle_translation_error` and counted as a failure it wasn't. `cancel_event` is threaded in by `PDFProcessor` (one `threading.Event` per job, popped in `BaseTranslator.__init__` like the other cross-cutting kwargs), so a Cancel click stops new LLM calls within one in-flight paragraph per worker thread — the asyncio-level `task.cancel()` alone never reached code already running synchronously on a BabelDOC worker thread. Argos gets the same top-of-`translate()` cancel check but no rate limiter or backoff — it's local, and the existing "a late `event.set()` on an abandoned batch entry is harmless" behavior already covers it.
