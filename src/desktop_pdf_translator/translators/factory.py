...
class TranslatorFactory:
    ...
    @classmethod
    def get_service_info(cls, service: TranslationService) -> Dict:
        ...
        try:
            ...
            info["available"] = availability_map.get(service, False)
            
            if info["available"]:
                # Create test instance to get more info
                translator = cls.create_translator(
                    service=service,
                    lang_in="en", 
                    lang_out="vi"
                ...
                info["supported_models"] = translator.get_supported_models()
                
                # Check configuration
                is_configured, config_message = translator.validate_configuration()
                info["configured"] = is_configured
                if not is_configured:
                    info["error"] = config_message
