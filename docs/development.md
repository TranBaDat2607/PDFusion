...
How to build and run PDFusion from source. If you just want to use the app,
[the README](../README.md) has the installers.

PDFusion is a **Tauri 2 (Rust shell) + React + TypeScript + Tailwind + shadcn/ui**
desktop app. The Python translation/RAG backend runs as a **FastAPI sidecar**
that the Tauri shell spawns on app start. For why the pieces fit together the
way they do, see [architecture notes](architecture-notes.md).
...
| Packaged by CI | `.exe` (NSIS) | `.deb` | not yet |
...
Pointing PDFusion at a local or OpenAI-compatible model (Ollama, LM Studio, a
proxy) is a runtime setting, not a build step —
[the README covers it](../README.md#use-your-own-model).
...
- **The `.deb` depends on `libwebkit2gtk-4.1-0` and `libgtk-3-0`**, derived by
  the bundler rather than hand-listed, and takes the webview from the system.
  **There is no AppImage** — see *Why there is no AppImage* in the
...
- **The bundles are large.** v1.1.1 shipped a 458 MB `.exe` and a 723 MB `.deb`;
  the staged sidecar tree alone is ~1.2 GB before compression. Most of the rest
  is the ~290 MB of engine assets it ships so the app works offline on first
  run. The RAG embedding weights (~470 MB) are *not* in there — they download
  lazily on first Chat use to `~/.cache/huggingface`.
...
│   └── src-tauri/                    ← Rust shell, sidecar lifecycle, icons
├── src/desktop_pdf_translator/
│   ├── api/                          ← FastAPI sidecar
│   ├── config/                       ← TOML + .env settings
│   ├── processors/                   ← BabelDOC translation pipeline
│   ├── translators/                  ← Argos (offline) / OpenAI / Gemini / Anthropic
│   ├── rag/                          ← ChromaDB + RAG chain + hybrid retrieval
│   ├── storage/                      ← SQLite records and migrations
│   └── utils/                        ← API key encryption, paths, logging
├── scripts/                          ← build_sidecar, fetch_offline_assets, render_icon
├── assets/branding/                  ← icon source of truth (see render_icon.py)
├── docs/                             ← this file + architecture-notes.md
├── pdfusion-sidecar.spec             ← PyInstaller spec
